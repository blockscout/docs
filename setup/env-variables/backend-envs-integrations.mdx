...
| -------- | ----------- | ---------- |
| `MICROSERVICE_HTTP_POOL_SIZE` | Size of the keep-alive connection pools used by `Explorer.MicroserviceInterfaces.HttpClient` for BENS and Metadata requests. Two pools are used: `:microservices` for short requests on the critical path and `:microservices_proxy` for requests proxied to a microservice on behalf of an API caller, so a handful of long-running proxied requests can no longer starve the preloads. Both pools are supervised from `Explorer.Application` and sized by this variable. Implemented in [#14689](https://github.com/blockscout/blockscout/pull/14689) | Version: master <br />Default: `1000` <br />Applications: API |
...
| `NOVES_FI_API_TOKEN`    | [Noves.fi API](https://blockscout.noves.fi/swagger/index.html) token. Implemented in [#9056](https://github.com/blockscout/blockscout/pull/9056).      | Version: v6.1.0+ <br />Default: (empty) <br />Applications: API                       |
                                                                                                                                                                         | Version: v6.6.0+ <br />Default: `50` <br />Applications: API                              |
