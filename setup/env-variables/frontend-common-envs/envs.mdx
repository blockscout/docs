---
title: "Frontend ENVs"
description: "Complete reference of Blockscout frontend environment variables for app config, API, UI, features, marketplace, and rollup integrations."
---
...
_Note_ URL templates should contain placeholders of NFT hash (`{hash}`) and NFT id (`{id}`). These placeholders will be substituted with particular values for every collection or instance.
...
| NEXT_PUBLIC_ROLLUP_L1_BASE_URL                 | `string`                                                                                                                 | Blockscout base URL for L1 network. **DEPRECATED** **`Use NEXT_PUBLIC_ROLLUP_PARENT_CHAIN instead`**                                                                                                                                                                                                         | Required                          | -             | `http://eth-goerli.blockscout.com`                             | v1.24.0+ |
...
| NEXT_PUBLIC_ROLLUP_PARENT_CHAIN_NAME           | `string`                                                                                                                 | Set to customize L1 transaction status labels in the UI (e.g., "Sent to "). This setting is applicable only for Arbitrum-based chains. **DEPRECATED** **`Use NEXT_PUBLIC_ROLLUP_PARENT_CHAIN instead`**                                                                                                      | -                                 | -             | `DuckChain`                                                    | v1.37.0+ |
...
| NEXT_PUBLIC_ROLLUP_DA_CELESTIA_NAMESPACE       | `string`                                                                                                                 | Hex-string for creating a link to the transaction batch on the Celenium explorer. "0x"-format and 60 symbol length. Available only for Arbitrum roll-ups.                                                                                                                                                   | -                                 | -             | `0x00000000000000000000000000000000000000ca1de12a9905be97beaf` | v1.38.0+ |
| NEXT_PUBLIC_ROLLUP_DA_CELESTIA_CELENIUM_URL    | `string`                                                                                                                 | URL for the Celenium explorer. It is used to create links to the Data Availability Blobs page. The URL should contain the full path without any search parameters related to the blob, as these will be constructed at runtime for each blob separately. Available only for Optimistic or Arbitrum roll-ups. | -                                 | -             | `https://mocha.celenium.io/blob`                               | v2.0.2+  |
...
_Note_ The url to the original token will be constructed as `<base_url>/<token_hash>`, e.g. `https://eth.blockscout.com/token/<token_hash>`
