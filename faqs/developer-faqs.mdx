...
<Accordion title="How do I fix indexer timeouts or update memory to fix indexer memory errors?">
* More on [indexer timeouts](/faqs/developer-faqs#undefined)

* More on [updating memory consumption](/setup/indexing/how-do-i-update-memory-consumption-to-fix-indexer-memory-errors)

</Accordion>

<Accordion title='How to incorporate "Add to MM" functionality'>
...
<Accordion title="How do I update the internal tx fetcher to skip blocks or start from block to prevent errors?">
Set `TRACE_FIRST_BLOCK` and `TRACE_LAST_BLOCK` [env variables](/setup/env-variables).

</Accordion>

<Accordion title="What is the command to install the smart contract verifier?">
`cargo install --git` [https://github.com/blockscout/blockscout-rs](https://github.com/blockscout/blockscout-rs) `smart-contract-verifier-http` [Learn more about the contract verifier](/setup/microservices/smart-contract-verification)

</Accordion>

<Accordion title='How to fix error: "Could not compile dependency :libsecp256k1, "mix compile" failed.'>
