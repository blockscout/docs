...
description: "REST endpoint that returns human-readable summaries of Blockscout transactions and addresses using decoded contract calls and metadata."
---

### https\://eth.blockscout.com/api/v2/transactions/transaction\_hash/summary
...
4. **Data Processing** : The retrieved transaction data is processed, involving multiple phases:
   ...
   2. **LLM Phase** : If the type of action cannot be determined algorithmically in the Pre-LLM phase, the service leverages a Large Language Model (LLM) to infer the type of on-chain action.
