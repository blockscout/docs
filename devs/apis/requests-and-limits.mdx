...
description: "Blockscout API rate limits and environment variables for per-instance API keys, including IP-based defaults and plan-specific request limits."
...
| --- | --- | --- | --- | --- |
| No API key (by IP) | 300 req/min (default) |  | `API_RATE_LIMIT_BY_IP` & `API_RATE_LIMIT_BY_IP_TIME_INTERVAL` | The default rate limit is 300 requests per minute per IP (`API_RATE_LIMIT_BY_IP=300`, `API_RATE_LIMIT_BY_IP_TIME_INTERVAL=1m`). Older instances used 3 requests per minute. This limit applies to all requests that are not from whitelisted addresses and do not use an API key. |
| Temp token | 3 |  | `API_RATE_LIMIT_UI_V2_WITH_TOKEN` & `API_RATE_LIMIT_UI_V2_WITH_TOKEN_TIME_INTERVAL` | When a frontend user reaches the default IP limit, they must solve a captcha to obtain a temporary token. This token provides a personal (not per-IP) rate limit of 5 req/sec. Token TTL is configured via `API_RATE_LIMIT_UI_V2_TOKEN_TTL` |
...
See the [backend ENV variables](/setup/env-variables/backend-env-variables) page for the full list of `API_RATE_LIMIT_*` variables.