...
| --- | --- | --- | --- | --- |
| No API key (by IP) | 3 (old per-instance value) |  | `API_RATE_LIMIT_BY_IP` & `API_RATE_LIMIT_BY_IP_TIME_INTERVAL` | The default rate limit is 3 requests per minute. This limit applies to all requests that are not from whitelisted addresses and do not use an API key. |
