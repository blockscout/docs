---
title: "ETH RPC API"
description: "Reference for the 16 Ethereum JSON-RPC 2.0 methods supported by Blockscout, available via the PRO API and per-instance eth-rpc endpoints."
---
...
```text
https://api.blockscout.com/{instance}/json-rpc
```
...
```bash
curl -X POST https://eth.blockscout.com/api/eth-rpc \
  -H "Content-Type: application/json" \
  -d '{
    "jsonrpc": "2.0",
    "method": "eth_call",
    "params": [
      { "to": "0xCONTRACT_ADDRESS", "data": "0x7b0a47ee" },
      "latest"
    ],
    "id": 1
  }'
...
The `result` is the ABI-encoded return value, right-padded to 32 bytes. For a `uint256` return, parse the full hex string as an integer.
...
curl -X POST https://eth.blockscout.com/api/eth-rpc \
  ...
  -d '{
    ...
    "params": [
      "0xPROXY_ADDRESS",
      "0x360894a13ba1a3210667c828492db98dca3e2076cc3735a920a3ca505d382bb3",
      "latest"
    ],
    "id": 1
  }'
...
The result is 32 bytes; the implementation address is the last 20 bytes (strip the leading zero padding).

If the proxy is already verified in Blockscout, the [`getsourcecode`](/devs/apis/rpc/contract) contract RPC action returns `ImplementationAddress` and `IsProxy` directly, so you can skip the storage read.
