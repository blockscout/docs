...
description: "Query Blockscout blockchain data with GraphQL, a typed query language that lets clients request the exact transaction and address fields they need."
...
3\) **Subscription:** subscribe to real-time data.
...
To access Blockscout GraphQL interface you can use [GraphiQL](https://eth.blockscout.com/api-docs?tab=graphql_api) - an in-browser IDE for exploring GraphQL. It's built into Blockscout. From the `APIs` dropdown menu choose `GraphQL`.
...
```sh
curl 'https://eth-sepolia.blockscout.com/api/v2/graphql'
