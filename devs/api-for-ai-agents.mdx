...
description: "Give AI agents structured onchain data across 100+ EVM chains with Blockscout PRO API and MCP server for decoded transactions and metadata."
icon: "user-robot"
---
...
**x402** provides per-call infrastructure for autonomous agents. Access the same Pro API endpoints as a key-based request, billed per call instead of per account. [More info and example setup](/devs/x402-payments).
...
3. **For x402**: See the [x402 payments page](/devs/x402-payments)
4. **Need help?** Join the [Blockscout Discord](https://discord.gg/blockscout) — the dev team is active and responsive
