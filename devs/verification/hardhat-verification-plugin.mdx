---
title: "Verify contracts with the Hardhat plugin on Blockscout"
description: "Verify smart contracts on Blockscout with the hardhat-verify plugin, including setup steps for Hardhat 3 native support and Hardhat 2 configuration."
...
A key is required for any publicly hosted Blockscout chain. A private, self-hosted instance or Autoscout instance is a separate case and may not require one. Add your Pro API key under `verify.blockscout`:
...
<Warning>
  This is Hardhat 2 syntax — install with `@nomicfoundation/hardhat-verify@hh2`. There's no native Blockscout shortcut on this path, you configure `customChains` manually, as shown below. A Pro API key is now required for any publicly hosted Blockscout chain.
</Warning>
...
Your basic [Hardhat config file](https://hardhat.org/config/) (`hardhat.config.js` or `hardhat.config.ts`) will be set up to support the network you are working on. In this example we use the Optimism Sepolia test network and a `.ts` file.
...
// Get a free key at https://dev.blockscout.com/
...
const config: HardhatUserConfig = {
  ...
  etherscan: {
    apiKey: {
      'optimism-sepolia': BLOCKSCOUT_API_KEY
    },
    customChains: [
      {
        ...
        chainId: CHAIN_ID,
        urls: {
          apiURL: `https://api.blockscout.com/${CHAIN_ID}/api`,
...
// Add a new chain by adding one line here.
const CHAINS = [
...
const config: HardhatUserConfig = {
  ...
  etherscan: {
    apiKey: Object.fromEntries(
      CHAINS.map(({ network }) => [network, BLOCKSCOUT_API_KEY])
    ),
...
<Accordion title="Do I need a Pro API key for each chain?">
  Yes, but one Pro API key covers every hosted chain. On Hardhat 2, each network still needs its own entry in the `apiKey` object (the network name in `customChains` must match a key in `apiKey`), but the value should be the same real key repeated for every network.
</Accordion>

<Accordion title="Can I use the Pro API with hardhat-verify?">
  Yes, on both. Hardhat 3's `hardhat-verify@3.1.0+` has a native shortcut — pass your key via `verify.blockscout.apiKey` and the plugin handles routing for you. Hardhat 2 (`@hh2`) has no such shortcut, but reaches the same Pro API by configuring `customChains` manually. Point `apiURL` at `https://api.blockscout.com/CHAIN_ID/api` (replacing `CHAIN_ID` with the target chain's numeric ID) and set a real key under `etherscan.apiKey`, as shown above.
</Accordion>

<Accordion title="Do self-hosted or Autoscout instances need a Pro API key?">
  No. A self-hosted Blockscout instance, or one you've spun up with Autoscout, runs its own singular API and isn't part of the Pro API network — no key required. Point `apiURL` at that instance's own address (e.g. `https://your-instance.example/api`) instead of `api.blockscout.com`.
</Accordion>
