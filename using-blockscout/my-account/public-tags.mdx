...
Because a public tag is attached to an address (not to the verified contract name), the two can differ. For example, a verified contract named `DexRouter` may render as `OKX Labs: DexRouter` on transaction and address pages when the underlying address carries a `OKX Labs: DexRouter` public tag. The contract name is unchanged; the tag adds project attribution.
