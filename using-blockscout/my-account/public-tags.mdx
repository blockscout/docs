---
title: "Public Tags: Request and Report Address Labels"
description: "Request public tags for your project's contracts or report incorrect labels on Blockscout, improving address discoverability and search visibility."
---
...
Approved public tags appear on the address page itself, next to the address name or contract name in the header (see [Address Page](/using-blockscout/overviews/address-page)). If the header shows no tag badge, the address does not have an approved public tag on that instance.

Because a public tag is attached to an address (not to the verified contract name), the two can differ. For example, a verified contract named `DexRouter` may render as `OKX Labs: DexRouter` on transaction and address pages when the underlying address carries an `OKX Labs: DexRouter` public tag. The contract name is unchanged; the tag adds project attribution.
...
See the [PRO API overview](/devs/pro-api) for authentication and base URL details.
