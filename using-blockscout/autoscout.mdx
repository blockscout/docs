...
- **HTTP RPC URL\***: The **archive node** url (https://your-rpc-url) [___Note: archive node is required for internal transactions and token/coin historical balances. Tracing should be enabled for internal transactions.___](/setup/requirements/node-tracing-json-rpc-requirements)
